...
class Trainer(object):
    ...
    def cycle_dataset(self, mode):
        if mode == 'train':
            for vos_data in self.train_loader:
                ...
                loss = nn.CrossEntropyLoss()(vos_out['scores'].view(-1, 2, H, W), gt_masks.reshape(-1, H, W))
                ...
                iou = torch.mean(get_iou(vos_out['scores'].view(-1, 2, H, W), gt_masks.reshape(-1, H, W)).view(B, -1, 2)[:, :, 1:])
    ...
    def save_checkpoint(self, alt_name=None):
        if alt_name is not None:
            file_path = 'trained_model/{}_{}.pth'.format(self.save_name, alt_name)
        else:
            file_path = 'trained_model/{}_{:04d}.pth'.format(self.save_name, self.epoch)
        torch.save(self.model.state_dict(), file_path)

    def load_checkpoint(self, mode, epoch, gpu):
        checkpoint_path = 'trained_model/{}_{:04d}.pth'.format(mode, epoch)
